...
let ZooAnimalNames = zooAnimals.forEach(function(i) {
...
let alllowercase = zooAnimals.map((names) => {
...
let animalPop = zooAnimals.filter((lowerpop) => {
  return lowerpop.population < 5
});

largerPopulation.push(animalPop);
...
*/
const populationTotal = zooAnimals.reduce((zoototal, animpop) => {
  return zoototal + animpop.population;
}, 0);
